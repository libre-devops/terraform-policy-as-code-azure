param (
    ...
    [string]$UseCheckovJson = "false",
    [string]$TerraformDestroyPlanFileName = "tfplan-destroy.plan",
    [string]$TerraformCodeLocation = "tests/builds/checkov/resource-naming",
    [string]$ExternalCheckovChecksDir = "checkov"
)
...
$fullExternalCheckovChecksDir = Join-Path -Path $currentWorkingDirectory -ChildPath $ExternalCheckovChecksDir
...
try
{
    ...
    $convertedUseCheckovJson = ConvertTo-Boolean $UseCheckovJson
    ...
    if ($convertedRunTerraformPlan -or $convertedRunTerraformPlanDestroy)
    {
        ...
        if ($convertedRunTerraformPlan)
        {
            ...
            {
                if ($convertedUseCheckovJson)
                {
                    Invoke-CheckovFlexible -Path "$fullTerraformCodePath/$TfPlanFileName" -ExternalChecksDir $fullExternalCheckovChecksDir
                }
                else
                {
                    Invoke-CheckovFlexible -Path $fullTerraformCodePath -ExternalChecksDir $fullExternalCheckovChecksDir
                }
            }
        }
